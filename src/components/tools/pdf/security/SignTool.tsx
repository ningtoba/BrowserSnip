 ...
 *
 * Prepare content here first (type text, upload image, draw),
 * then click "Add to PDF" to place it on the current page.
 * The overlay (SignMain) handles positioning, resizing, and editing.
 */
import { useState, useEffect, useCallback, useRef } from 'react';
import { usePDFEngine } from '@/hooks/usePDFEngine';
import {
  getSignState, setSignPage, addSigItem, updateSigItem, removeSigItem,
...
export function SignTool() {
  ...
  // ── Preparation state (sidebar-local, not yet on PDF) ──
  ...
  return (
    <div className="doodle-section space-y-3">
      ...
      {/* ═══════════════ Text ═══════════════ */}
      <div className="space-y-1.5 border-b border-cream-border pb-2">
        <p className="text-[10px] font-medium text-ink-muted">Text Signature</p>
        ...
        <div className="grid grid-cols-2 gap-1.5">
          ...
          <input type="color" value={textColor} onChange={(e) => setTextColor(e.target.value)}
            className="w-full h-6 rounded bg-cream border border-cream-border cursor-pointer" />
          ...
          <input type="number" value={textH} min={20} max={100}
            onChange={(e) => setTextH(Number(e.target.value))}
            className="doodle-input text-center text-[10px]" placeholder="H" />
        </div>
      ...
      {/* ═══════════════ Image ═══════════════ */}
      <div className="space-y-1.5 border-b border-cream-border pb-2">
        <p className="text-[10px] font-medium text-ink-muted">Image Signature</p>
      ...
      {/* ═══════════════ Draw ═══════════════ */}
      <div className="space-y-1.5 border-b border-cream-border pb-2">
        <p className="text-[10px] font-medium text-ink-muted">Hand-drawn Signature</p>
      ...
      {selected && (
        <div className="space-y-2 border-t border-cream-border pt-2">
      ...
      {st.items.length > 0 && (
        <div className="border-t border-cream-border pt-2">
          <p className="text-[10px] font-medium text-ink-muted mb-1.5">
            Placed Signatures ({st.items.length})
          </p>
          <div className="space-y-1 max-h-36 overflow-y-auto">
            {st.items.map((item) => (
              <div key={item.id}
                onClick={() => selectSigItem(item.id)}
                className={`flex items-center gap-2 p-1.5 rounded cursor-pointer text-[10px] transition-colors
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-ink-muted/50">p.{item.page}</div>
                </div>
                <button
